            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @foreach ($AkunUsers as $akunUser)
                            @if ($akunUser->hak_akses === 'mahasiswa')
                                <tr class="hover:bg-slate-50/50 transition-colors group">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span
                                                class="font-bold text-slate-800 text-sm">{{ $akunUser->nama_peminjam }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600 font-medium">
                                        <button class="p-2 text-slate-900 hover:text-blue-500"
                                            @click="OpenImgIdentitas = true; selectedPeminjam = {
                                        no_identitas: '{{ $akunUser->no_identitas }}',
                                        img: '{{ $akunUser->img_identitas }}',
                                    }">
                                    ...
                                    <td
                                        class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-700 capitalize">
                                        {{ $akunUser->prodi }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($akunUser->status === 'active')
                                            <span
                                                class="flex items-center gap-1.5 text-[12px] font-bold text-emerald-600">
                                ...
                                <tr class="hover:bg-slate-50/50 transition-colors group">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span
                                                class="font-bold text-slate-800 text-sm">{{ $akunUser->nama_dosen ?? $akunUser->nama_staff }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600 font-medium">
                                        -
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @else
                                            <span
                                                class="flex items-center gap-1.5 text-[12px] font-bold text-gray-600">
                                                <span class="w-1.5 h-1.5 rounded-full bg-gray-500"></span>
                                                None Active
                                            </span>
                                        @endif
                ...
                <div class="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
                    ...
                    <div class="text-sm text-slate-500 font-medium">
                        Menampilkan
                        <span class="text-slate-900 font-bold">{{ $AkunUsers->count() }}</span>
                        dari
                        <span class="text-slate-900 font-bold">{{ $AkunUsers->total() }}</span>
                        data
                    </div>

                    <div class="flex gap-2">
                        {{-- Tombol ke Halaman Sebelumnya --}}
                        @if ($AkunUsers->onFirstPage())
                            <button
                        ...
                        @else
                            <a href="{{ $AkunUsers->previousPageUrl() }}"
                        ...
                        {{-- Nomor Halaman --}}
                        @foreach ($AkunUsers->getUrlRange(1, $AkunUsers->lastPage()) as $page => $url)
                            @if ($page == $AkunUsers->currentPage())
                                {{-- Halaman Aktif --}}
                                <button
                        ...
                        @if ($AkunUsers->hasMorePages())
                            <a href="{{ $AkunUsers->nextPageUrl() }}"
                                class="w-10 h-10 border border-slate-200 rounded-lg flex items-center justify-center text-slate-400 hover:text-primary hover:border-primary transition-all bg-white">
