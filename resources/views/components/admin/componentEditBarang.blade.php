        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            @foreach ($DataBarang as $ruang)
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <form action="{{ route('edit-barang-informasi-dasar') }}" enctype="multipart/form-data"
                        method="post">
                        @csrf
                        <div class="p-6 space-y-5">
                            <div>
                                <div class="space-y-2">
                                    <label class="text-sm font-semibold text-slate-700">ID Barang</label>
                                    <input name="id_item"
                            ...
                            <div class="grid grid-cols-1 gap-5" x-data="imageUploader('{{ Storage::disk('s3')->url(str_replace('//', '/', $ruang->img_item)) }}')">
                                <div class="border-2 border-dashed rounded-xl flex flex-col items-center justify-center p-8 transition-all group min-h-[400px]"
                                    ...
                                    <input type="file" name="gambar_item" id="fileInput" class="hidden"
                ...
                <div class="space-y-4">
                    @foreach ($DataBarang as $ruang)
                        ...
                        <form action="{{ route('edit-barang-visibility') }}" method="post">
                            ...
                            <div x-data="{ tersedia: {{ $ruang->visibility_item === '1' ? 'true' : 'false' }} }"
                                :class="tersedia ? 'bg-green-50 text-green-700 border-green-100' :
                                    'bg-red-50 text-red-700 border-red-100'"
                                class="flex items-center justify-between p-3 rounded-xl border transition-all">
