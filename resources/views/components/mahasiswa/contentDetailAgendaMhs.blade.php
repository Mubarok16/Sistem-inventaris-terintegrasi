            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-200"
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <!-- Barang -->
                        @if ($usage_item != null)
                            @foreach ($usage_item as $usage_item)
                                {{-- @if ($usage_item != null) --}}
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="size-12 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 overflow-hidden bg-cover bg-center"
                                                ...
                                                style="background-image: url('{{ Storage::disk('s3')->url(str_replace('//', '/',$usage_item->img_item)) }}')">
                                        ...
                                        @if ($usage_item->jam_mulai_usage_item != null && $usage_item->jam_selesai_usage_item != null)
                                            <div class="flex flex-col gap-1">
                                                <div class="flex items-center gap-2 text-sm text-slate-600">
                                                    ...
                                                    <span>
                                                        {{-- Setiap hari --}}
                                                        {{ date('d M y', strtotime($usage_item->tgl_pinjam_usage_item)) }}
                                                    </span>
                                                </div>
                                                <div class="flex items-center gap-2 text-sm text-slate-500">
                                                    <i class="fa-solid fa-clock text-base"></i>
                                                    <span>
                                                        {{ date('H:i', strtotime($usage_item->jam_mulai_usage_item)) }}
                                                        -
                        ...
                        <!-- rauang -->
                        @if ($usage_room != null)
                            @foreach ($usage_room as $usage_room)
                                {{-- @if ($usage_room != null) --}}
                                <tr class="group hover:bg-slate-50 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="size-12 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 overflow-hidden bg-cover bg-center"
                                                data-alt="Meeting Room Interior"
                                                style="background-image: url('{{ Storage::disk('s3')->url(str_replace('//', '/',$usage_room->gambar_room)) }}')">
                                            </div>
                                            <div>
                                                <p class="font-medium text-slate-900">
                                        ...
                                        @if ($usage_room->jam_mulai_usage_room != null && $usage_room->jam_selesai_usage_room != null)
                                            <div class="flex flex-col gap-1">
                                                <div class="flex items-center gap-2 text-sm text-slate-600">
                                                    ...
                                                    <span>
                                                        {{-- Setiap hari --}}
                                                        {{ date('d M y', strtotime($usage_room->tgl_pinjam_usage_room)) }}
                                                    </span>
                                                </div>
                                                <div class="flex items-center gap-2 text-sm text-slate-500">
                                                    <i class="fa-solid fa-clock text-base"></i>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 pt-4 border-t border-slate-100">
                        @foreach ($dataDetailPengajuanPeminjaman as $dataPeminjaman)
                            ...
                            <div class="items-center gap-4 mb-2">
                                ...
                                @if ($dataPeminjaman->status_peminjaman === 'dipinjam' || $dataPeminjaman->status_peminjaman === 'selesai' || $dataPeminjaman->status_peminjaman === 'terlambat' || $dataPeminjaman->status_peminjaman === 'dibatalkan' || $dataPeminjaman->status_peminjaman === 'ditolak')
                                @else
