            ...
            <div class="flex flex-col gap-3">
                <div>
                    ...
                    <div class="flex items-center gap-2 mt-2">
                        ...
                        <span
                            class="text-green-500  text-md font-medium leading-normal">({{ $item->kondisi_item }})</span>
                    </div>
                </div>
                ...
                <form action="{{ route('mahasiswa-add-cart-barang') }}" method="post">
                    @csrf
                    <!-- Actions -->
        ...
        <div class="border-b border-gray-200">
            <nav aria-label="Tabs" class="-mb-px flex gap-8 overflow-x-auto hide-scrollbar no-underline!">
                <a aria-current="page"
                    ...
                    href="#">
                    {{ date('l, d F Y', strtotime($tglForTblUsageBrng)) }}
                </a>
            </nav>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        ...
                        @else
                            @foreach ($dataUsageItems as $item)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4 font-bold text-[#111418]">
                                        {{ date('H:i', strtotime($item->tgl_pinjam_usage_item)) }} -
                                        {{ date('H:i', strtotime($item->tgl_kembali_usage_item)) }}
                                    </td>
                                    <td class="px-6 py-4">
